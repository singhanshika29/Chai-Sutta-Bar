...
$budget = trim($_POST["liquid-capital"] ?? '');
$net = trim($_POST["net"] ?? '');
...
if($nerr === '' && $eerr === '' && $merr === '') {
  ...
  if ($mysqli->connect_errno) {
    ...
    $stmt = $mysqli->prepare("INSERT INTO franchise (name, email, phone, city, budget, worth, message) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssss", $name, $email, $phone, $city, $budget, $net, $message);
    if($stmt->execute()){
